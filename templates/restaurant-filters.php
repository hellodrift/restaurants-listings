...
<form class="restaurant_filters">
	...
	<div class="search_restaurants">
		...
		<div class="search_keywords">
			...
			<input type="text" name="search_keywords" id="search_keywords" placeholder="<?php esc_attr_e( 'Keywords', 'restaurants-listings' ); ?>" value="<?php echo esc_attr( $keywords ); ?>" />
		</div>

		<div class="search_location">
			<label for="search_location"><?php _e( 'Location', 'restaurants-listings' ); ?></label>
			<input type="text" name="search_location" id="search_location" placeholder="<?php esc_attr_e( 'Location', 'restaurants-listings' ); ?>" value="<?php echo esc_attr( $location ); ?>" />
		</div>
		...
		<?php elseif ( $show_categories && ! is_tax( 'restaurant_listing_category' ) && get_terms( 'restaurant_listing_category' ) ) : ?>
			<div class="search_categories">
				<label for="search_categories"><?php _e( 'Category', 'restaurants-listings' ); ?></label>
				...
				<?php else : ?>
					<?php listings_dropdown_categories( array( 'taxonomy' => 'restaurant_listing_category', 'hierarchical' => 1, 'show_option_all' => __( 'Any category', 'restaurants-listings' ), 'name' => 'search_categories', 'orderby' => 'name', 'selected' => $selected_category, 'multiple' => false ) ); ?>
				<?php endif; ?>
			</div>
		<?php endif; ?>
...
<noscript><?php _e( 'Your browser does not support JavaScript, or it is disabled. JavaScript must be enabled in order to view listings.', 'restaurants-listings' ); ?></noscript>